...
const maleTheme = {
  ...
  sectionHeader: 'rgba(59,130,246,0.06)',
};

const femaleTheme = {
  dot: 'bg-pink-400',
  accent: 'text-pink-500',
  sectionHeader: 'rgba(236,72,153,0.06)',
};
...
function DetailRow({ label, value, empty }: { label: string; value?: string | number | null; empty?: string }) {
  return (
    <div className="flex items-start justify-between gap-4 py-2.5 border-b last:border-0" style={{ borderColor: 'var(--border-default)' }}>
      <span className="text-sm whitespace-nowrap" style={{ color: 'var(--text-muted)' }}>{label}</span>
...
function Section({ title, icon, theme, children }: { title: string; icon?: React.ReactNode; theme?: typeof maleTheme; children: React.ReactNode }) {
  return (
    <Card className="overflow-hidden">
      <CardHeader style={{ background: theme?.sectionHeader || 'var(--gray-50)' }}>
        <div className="flex items-center gap-2">
          {icon && <span className={theme?.accent || 'text-[var(--gold)]'}>{icon}</span>}
          <h3 className="text-sm font-semibold tracking-wide" style={{ color: 'var(--text-primary)' }}>{title}</h3>
        </div>
      </CardHeader>
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <Link href="/admin/profiles" className="flex items-center gap-2 text-sm transition-colors" style={{ color: 'var(--text-muted)' }}>
      ...
      <Card className="overflow-hidden">
        <div className="p-6 space-y-3">
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm" style={{ color: 'var(--text-muted)' }}>
            <span className="font-mono" style={{ color: 'var(--text-muted)' }}>{profile.profile_id}</span>
      ...
      {profile.photo_urls && profile.photo_urls.length > 0 && (
        <Card>
          <CardHeader>
            <h3 className="text-sm font-semibold tracking-wide" style={{ color: 'var(--text-primary)' }}>Photos</h3>
          </CardHeader>
          <CardContent>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <Section title="Family Details" theme={theme}>
          ...
          {siblings && siblings.length > 0 && (
            <div className="py-2.5 border-b last:border-0" style={{ borderColor: 'var(--border-default)' }}>
              <span className="text-sm" style={{ color: 'var(--text-muted)' }}>Siblings</span>
              <div className="mt-2 space-y-2">
                {siblings.map((sib, i) => (
                  <div
                    key={i}
                    className="flex items-center justify-between rounded-xl px-3 py-2"
                    style={{ background: isMale ? 'rgba(59,130,246,0.06)' : 'rgba(236,72,153,0.06)' }}
                  >
                    <div>
                      <p className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>{sib.name}</p>
                      <p className="text-xs capitalize" style={{ color: 'var(--text-muted)' }}>{sib.gender} &middot; {sib.marital_status === 'married' ? 'Married' : 'Unmarried'}</p>
                    </div>
                    {sib.occupation && (
                      <span className="text-xs" style={{ color: 'var(--text-secondary)' }}>{sib.occupation}</span>
                    )}
                  </div>
                ))}
              </div>
      ...
      {shares && shares.length > 0 ? (
        <Card>
          <CardHeader>
            <h3 className="text-sm font-semibold tracking-wide" style={{ color: 'var(--text-primary)' }}>Share History</h3>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-sm text-center py-4" style={{ color: 'var(--text-muted)' }}>No share history available</p>
          </CardContent>
        </Card>
      )}
    </div>
